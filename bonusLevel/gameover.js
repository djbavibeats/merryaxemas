...
class Gameover
{
    ...
    update(deltaTime)
    {
        if(spirit <= 0)
        {
            ...
            if(isSound && !BGM.mute)
            {
                var scream = this.scene.sound.add('Gameover');
                scream.play();
                BGM.mute = true;
            }
        }
        ...
        if(this.bg.alpha >= 0.95 && !this.UIAdded)
        {
            ...
            /* var promotetext = this.scene.add.text(0, height * 0.15, 'Save our new album Welcome To Horrorwood! Click HERE!!', { fontFamily: "kong", fontSize: 0.054 * scaleFactor, color: "#0000ff" }).setInteractive({ cursor: "pointer" })
            ...
            var promotetext2 = this.scene.add.text(0, height * 0.15, 'Save our new album Welcome To Horrorwood! Click HERE!!', { fontFamily: "kong", fontSize: 0.054 * scaleFactor, color: "#ffff00" }).setInteractive({ cursor: "pointer" })
            .on("pointerdown", function (pointer) {
                window.location.href = "https://link.iceninekills.com/WTHSave";
            });
